...
question_template = """
Consider the following scientific field

Field: {name}
Description: {description}

Generate 5 questions that an expert in this field would be able to answer.
Do not enumerate the questions, just list them.
"""

def create_batch_file(categories, filename="batch_questions.jsonl"):
    """Create a JSONL file with batch requests for all categories"""
    tasks = []
    
    for idx, category in enumerate(categories):
        task = {
            "custom_id": f"category-{idx}-{category.code}",
            "method": "POST",
...
def retrieve_batch_results(batch_response):
    ...
    for raw_response in raw_responses:
        ...
        if json_response.get('error'):
            print(f"Error for category {category_code}: {json_response['error']}")
            continue
        ...
        try:
            ...
            print(f"Error parsing response for category {category_code}: {e}")
            continue
    
    return results

def save_results_to_csv(results, categories):
    ...
    print(f"- Average questions per category: {len(data) / len(results):.1f}")
...
def main():
    ...
    # Create batch file
    batch_filename = create_batch_file(categories)
    ...
    try:
        ...
        if batch_response and batch_response.status == "completed":
            ...
            # Save results to CSV
            save_results_to_csv(results, categories)
