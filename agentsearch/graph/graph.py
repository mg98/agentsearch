...
if __name__ == '__main__':
    NUM_AGENTS = 256        # Instead of 16
    CORE_NUM_QUESTIONS = 64  # Instead of 8
    NUM_QUESTIONS = 16       # Instead of 8
    K_MATCHES = 8
    ...
    all_questions = Question.all()#_from_cluster(TOPIC, total_questions_asked + 100)
    ...
    for i in range(100):
        ...
        for i, idx in enumerate(top_3_indices_gnn):
            ...
            # Normalize using the same scale as training data
            if graph_data.trust_max is not None and graph_data.trust_min is not None:
                if graph_data.trust_max == graph_data.trust_min:
                    grade = 0.5  # All scores were the same in training
                else:
                    grade = (raw_grade - graph_data.trust_min) / (graph_data.trust_max - graph_data.trust_min)
                    grade = max(0.0, min(1.0, grade))  # Clamp to [0,1]
            else:
                grade = raw_grade / 4.0  # Fallback based on observed max in training
            print(f"{Fore.YELLOW}GNN {i+1}:\t{agent.name} | Actual: {Fore.GREEN if grade > 0 else Fore.RED}{grade:.2f}{Style.RESET_ALL} | Predicted: {Fore.CYAN}{predicted_score:.4f}{Style.RESET_ALL}")
