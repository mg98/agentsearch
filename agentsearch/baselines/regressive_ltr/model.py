...
from agentsearch.baselines.regressive_ltr.utils import FeatureVector, K_VALUES
from agentsearch.utils.globals import get_torch_device
from torch.utils.data import TensorDataset, DataLoader
import wandb

class RegressiveLTRModel(nn.Module):
...
def train_model(
    ...
    model = RegressiveLTRModel().to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    criterion = nn.MSELoss()

    if use_wandb:
        wandb.init(
            project="agentsearch-regressive-ltr",
            config={
                "epochs": epochs,
                "lr": lr,
    ...
    if use_wandb:
        wandb.log({
            ...
            "val_samples": n_val,
            "avg_relevance_train": y_train.mean().item(),
            "avg_relevance_val": y_val.mean().item(),
            "std_relevance_train": y_train.std().item(),
            "std_relevance_val": y_val.std().item()
        })
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            train_loss_eval = criterion(train_predictions, y_train).item()

            train_mae = torch.abs(train_predictions - y_train).mean().item()
            val_mae = torch.abs(val_predictions - y_val).mean().item()
