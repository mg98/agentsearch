...
Creating the dataset is a multi-step process. The steps are documented below.

### 1. Scrape Agents

This will scrape all Google Scholar profiles with TU Delft affiliation, including names and self-proclaimed research fields (= agent card), until failure.

```
python -m scripts.scrape_scholar
```
...
This will generate a dataset of questions (1 per paper) and export it to `data/questions.csv`.

```
python -m scripts.extract_paper_titles
...
### 5. Simulate Performance Reports

```
python -m scripts.gen_reports
```
